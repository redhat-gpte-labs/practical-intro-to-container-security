...
REPOSITORY                            TAG      IMAGE ID       CREATED      SIZE
quay.io/bkozdemb/hello                latest  398a3b0bab81  7 months ago   250 MB
node1.pph67.internal:5000/mynamespace/hello  latest  398a3b0bab81  7 months ago
   250 MB
```
...
```
Error: error authenticating creds for "node1.pph67.internal:5000": error pinging docker registry node1.pph67.internal:5000: Get "https://node1.pph67.internal:5000/v2/": x509: certificate relies on legacy 
Common Name field, use SANs or temporarily enable Common Name matching with GODEBUG=x509ignoreCN=0
```

The `GODEBUG=x509ignoreCN=0` variable needs to be set.
```
